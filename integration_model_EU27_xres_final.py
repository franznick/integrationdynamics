...
def run_simulation(CP, use_EC=True, theta_mode="fixed", voting_rule="majority"):
    global omega_pol, omega_econ, omega_f, w
    ...
    for t in range(T):
        if theta_mode == "fixed":
            ...
            theta_c = np.clip(1.0 - CP[:, t], 0.0, 1.0)
        A[:, t] = theta_c * I[t]
        S_t = 1.0 - np.std(CP[:, t])
        for c in range(C):
        ...
        for c in range(C):
            IP[c, t] = (alpha_cp * CP_eff_t[c] + alpha_pf * PF[c, t] + ip * EC[t] - gamma_crc * CRC[c, t])
        ...
        if voting_rule == "majority":
            ...
            c_star = np.argmin(np.abs(Id_c[:, t]))
            Id_agg[t] = Id_c[c_star, t]
        sd_CP = np.std(CP[:, t])
        ...
        if use_EC:
            ...
            I_star[t+1] = I_star[t]
        I_next = I[t] + lambda_t[t] * Id_agg[t]
    ...
    t = T
    EC[t] = max(0.0, I_star[t] - I[t]) if use_EC else 0.0
    return {"I": I, "I_star": I_star, "EC": EC, "Id_agg": Id_agg, "lambda": lambda_t, "CP": CP, "alpha_p": alpha_p_ct, "betaE": betaE_ct, "CRC": CRC, "IP": IP, "SP": SP, "PF": PF}

def build_CP_symmetric_three_windows(shock_level=0.15):
    CP = np.zeros((C, T+1))
    windows = [(4,7),(14,17),(24,27)]
    for start, end in windows:
        CP[:, start:end] = shock_level
    return CP
